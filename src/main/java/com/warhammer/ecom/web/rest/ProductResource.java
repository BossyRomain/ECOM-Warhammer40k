...
/**
 * REST controller for managing {@link com.warhammer.ecom.domain.Product}.
 */
@RestController
...
public class ProductResource {
    ...
    public List<Product> getAllProducts(@RequestParam(name = "filter", required = false) String filter) {
        if ("commandline-is-null".equals(filter)) {
            LOG.debug("REST request to get all Products where commandLine is null");
            return StreamSupport.stream(productRepository.findAll().spliterator(), false)
                .filter(product -> product.getCommandLine() == null)
                .toList();
        }
