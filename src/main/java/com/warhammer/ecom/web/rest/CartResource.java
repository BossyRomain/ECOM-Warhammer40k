...
/**
 * REST controller for managing {@link com.warhammer.ecom.domain.Cart}.
 */
@RestController
...
public class CartResource {
    ...
    public ResponseEntity<Cart> partialUpdateCart(@PathVariable(value = "id", required = false) final Long id, @RequestBody Cart cart)
        ...
        Optional<Cart> result = cartRepository
            .findById(cart.getId())
            .map(existingCart -> {
                if (cart.getBuyingDate() != null) {
                    existingCart.setBuyingDate(cart.getBuyingDate());
                }
                if (cart.getPaid() != null) {
                    existingCart.setPaid(cart.getPaid());
                }
