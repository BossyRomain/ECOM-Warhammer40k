...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'releaseDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      releaseDate: product.releaseDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      releaseDate: restProduct.releaseDate ? dayjs(restProduct.releaseDate) : undefined,
    };
  }
